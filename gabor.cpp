...
QImage* printGabor()
{
	...
	Mat out_real, out_imag;

	for (int i = 0; i < 5; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			gabor.getFilterRealImagPart(frame, gabor.gaborRealKernels[i*j], gabor.gaborImagKernels[i*j], out_real, out_imag);
		}
	}

	normalize(out_real, out_real, 255, 0, CV_MINMAX, CV_8UC1);
	return Mat2QImage(out_real);
}
...
QImage* printGabor_2(int mu, int nu)
{
	...
	//output = out_real;
	//for (int a = 0; a < frame.total(); a++)
	//{
	//	output.at<float>(a) = sqrt(pow(out_real.at<float>(a), 2) + pow(out_imag.at<float>(a), 2));
	//}

	normalize(output, output, 255, 0, CV_MINMAX, CV_8UC1);
	return Mat2QImage(output);
}